...
ax_cv.scatter(cv_all, obs_mean_sojourn_all, lw=0.5, ls='-', s=10, alpha=0.8, color=plot_utils.dataset_color_dict[target_dataset])
...
ax_cv.plot(10**bins_mean_all_to_keep_no_nan, 10**bins_y_no_nan, lw=3 , c='k', ls='--')

#ax_mean.plot(10**bins_mean_all_to_keep_no_nan, 10**bins_y_no_nan, lw=2, c='k', ls='--')

ax_mean.set_xlabel("Mean relative abundance, " + r'$\bar{x}$', fontsize=14)
ax_cv.set_xlabel("CV of relative abundance, " + r'$\mathrm{CV}_{x}$', fontsize=14)

ax_mean.set_ylabel("Mean sojourn time (days), " + r'$\left < \mathcal{T} \right>$', fontsize=14)
ax_cv.set_ylabel("Mean sojourn time (days), " + r'$\left < \mathcal{T} \right>$', fontsize=14)


y_lim_min = 1.3
y_lim_max = 14

ax_cv.set_xlim([0.2, 2.01])

ax_mean.set_ylim([y_lim_min, y_lim_max])
ax_cv.set_ylim([y_lim_min, y_lim_max])
...
ax_mean.legend(loc = 'upper left', fontsize=11)
