...
x_cv_all = numpy.asarray(x_cv_all)
...
ax_sigma = inset_axes(ax, width="100%", height="100%", bbox_to_anchor=(0.56,0.64,0.35,0.35), bbox_transform=ax.transAxes, loc='upper right')
...
for dataset_i in data_utils.dataset_all:
    ...
    ax_sigma.scatter(x_cv_i, run_sojourn_integral_i, color=plot_utils.dataset_color_dict[dataset_i], alpha=0.6, s=10)
...
ax.set_ylabel("Sojourn trajectory height, " + r'$\mathcal{H}(\mathcal{T} \, )$', fontsize=17)
...
slope_sigma, intercept_sigma, r_value_sigma, p_value_sigma, std_err_sigma = stats_utils.log_log_regression(x_cv_all, run_sojourn_integral_all)

x_log10_range_sigma =  numpy.linspace(min(numpy.log10(x_cv_all)) , max(numpy.log10(x_cv_all)) , 10000)
#y_log10_fit_range = (slope*x_log10_range + intercept)

#y_log10_sigma = (-0.5*x_log10_range_sigma + intercept_sigma)
y_log10_sigma = ( x_log10_range_sigma + intercept_sigma)
...
ax_sigma.set_xlabel('CV of relative abundance\n(function of ' + r'$\sigma_{i}$' + ')', fontsize=12)
#bbox=dict(facecolor='white', edgecolor='none')
ax_sigma.set_ylabel(r'$\mathcal{H}(\mathcal{T}\,)$', fontsize=12)
...
#ax_sigma.set_xticklabels([], minor=False)
ax_sigma.tick_params(axis='x', which='both', labelbottom=False)
