    ...
    for host in host_all:
        ...
        for asv in  mle_dict[dataset][host].keys():
            ...
            if run_dict is not None:
                ...
                mean_area_all.append(calculate_mean_area(run_dict))
                asv_all.append(asv)
...
for idx in largest_area_all_idx:

    target_asv_dict[dataset_all_area[idx]][host_all_area[idx]].append(asv_all[idx])
...
for dataset_idx, dataset in enumerate(data_utils.dataset_all):
    ...
    for host_idx, host in enumerate(host_all):
        ...
        for key, value in mle_dict[dataset][host].items():
            ...
            if key in target_asv_dict[dataset][host]:
                lw=1.5
                alpha=1
                c='k'
                zorder=2
...
fig_name = "%strajectories_highest_area.png" % (config.analysis_directory)
